...
CREATE TABLE IF NOT EXISTS `juridico`.`Processos` (
  ...
  INDEX `fkprocessos_apensos_idx` (`processos_apensos` ASC),
  CONSTRAINT `fkprocessos_varas_nome`
    FOREIGN KEY (`varas_id`)
    REFERENCES `juridico`.`Varas` (`varas_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fkprocessos_apensos`
    FOREIGN KEY (`processos_apensos`)
...
START TRANSACTION;
USE `juridico`;
INSERT INTO `juridico`.`Pessoas` (`pessoas_id`, `usuarios_id`, `pessoas_cpf_cnpj`, `pessoas_rg`, `pessoas_nome`, `pessoas_datanasc`, `pessoas_email`, `pessoas_tel`, `pessoas_sexo`, `pessoas_oab`, `pessoas_endereco`, `pessoas_del`) VALUES (default, null, '46041283835', '456269502', 'Andrei', '1998-01-13', 'emersoncorrea@funprev.bauru.br', '14997016950', 'M', '12345', DEFAULT, 'N');
